...
int libjoy_name(int joy) {
    int result;
    result = string_new(SDL_GetJoystickInstanceName(joy));
    string_use(result);
    return result;
}
...
int libjoy_rumble(int joy, int low_frequency_rumble, int high_frequency_rumble, int duration) {
    if (joy >= 0 && joy < _max_joys) {
        ...
        return SDL_RumbleJoystick(_joysticks[joy],
...
void __pxtexport(libjoy, module_initialize)() {
    ...
    /* Open all joysticks */
    if ((_max_joys = num_joysticks) > MAX_JOYS) {
    ...
    for (i = 0; i < _max_joys; i++) {
        _joysticks[i] = SDL_OpenJoystick(i);
        if (!_joysticks[i])
